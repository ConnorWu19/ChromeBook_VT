...
LOG_FILE="/tmp/hp_stress.log"
echo "=== ChromeBook HP Stress Toolkit Session: $(date) ===" > "$LOG_FILE"
...
function startup_fade_in() {
    ...
    for c in "${fade_colors[@]}"; do
        ...
        echo "╔══════════════════════════════════════════════════════╗"
        echo "║          ChromeBook HP Stress Toolkit v1.0           ║"
...
function update_sysinfo() {
    ...
    if (( current_time - LAST_SYSINFO_CHECK >= 5 )); then
        ...
        if [[ -f /etc/lsb-release ]]; then
            ...
            local rev_ver=$(grep "CHROMEOS_RELEASE_BUILD_NUMBER" /etc/lsb-release | cut -d'=' -f2 | xargs)
...
function Run_Internal_SSD_Stress() {
    ...
    # 確認這裡的路徑有同步更新
    if [[ -f /usr/local/ChromeBook_HP_Stress_Toolkit/SSD/ssd.sh ]]; then
        sudo chmod +x /usr/local/ChromeBook_HP_Stress_Toolkit/SSD/ssd.sh
        sudo /usr/local/ChromeBook_HP_Stress_Toolkit/SSD/ssd.sh
    else
...
function Run_LinuxPCT_Stress() {
    ...
    local target_dir="/usr/local/ChromeBook_HP_Stress_Toolkit"
...
function Capture_PCT_Logs() {
    ...
    local log_dir="/usr/local/ChromeBook_HP_Stress_Toolkit/Log"
    if [[ -d "$log_dir" ]]; then
...
function draw_full_menu() {
    ...
    if [[ $SCALE_MODE -eq 0 ]]; then
        ...
        bot_border="╚══════════════════════════════════════════════════════╝"
        divider="────────────────────────────────────────────────────────"
        title1="║          ChromeBook HP Stress Toolkit v1.0           ║"
        ...
        case "$key" in
            ...
            "")
                ...
                case "$cmd" in
                    ...
                    CMD_EXIT) log_info "Exiting ChromeBook HP Stress Toolkit~Goodbye!"; exit 0 ;;
                esac
