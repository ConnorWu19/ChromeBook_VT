...
function Run_Internal_SSD_Stress() {
    ...
    # 確認這裡的路徑有同步更新
    if [[ -f /usr/local/ChromeBook_Validation_Toolkit/SSD/ssd.sh ]]; then
        sudo chmod +x /usr/local/ChromeBook_Validation_Toolkit/SSD/ssd.sh
        sudo /usr/local/ChromeBook_Validation_Toolkit/SSD/ssd.sh
    else
...
function Run_LinuxPCT_Stress() {
    ...
    local target_dir="/usr/local/ChromeBook_Validation_Toolkit"
...
function Capture_PCT_Logs() {
    ...
    local log_dir="/usr/local/ChromeBook_Validation_Toolkit/Log"
    if [[ -d "$log_dir" ]]; then
